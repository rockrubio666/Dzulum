...
def directoryBforce(url,f,verbose,cookie,agent, proxip,proxport,tor,report):
	...
	print colored('\nBeginning Directory Bruteforcing', 'green')
	fake = []
	conLen = []
	resources = []
	sites = []
...
def rep(rep,dos,index,url): #Reporte
	...
	for value in rep: 
		if 'text' in value: #Texto plano
			...
			fo.write('<?xml-stylesheet type="text/css" href="dforce.css"?>' + '\n')
			fo.write('<crawling>' + '\n')
